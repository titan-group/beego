...
func (this *JsonController) Get() {
	this.Data["Username"] = "titan-group"
...
func TestUrlFor2(t *testing.T) {
	...
	if handler.UrlFor("TestController.GetUrl", ":username", "titan-group") != "/v1/titan-group/edit" {
		Info(handler.UrlFor("TestController.GetUrl"))
		t.Errorf("TestController.List must equal to /v1/titan-group/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/titan-group", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "titan-group" {
		t.Errorf("post func should titan-group")
	}
}
